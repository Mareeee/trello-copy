...
export async function taskProgress(req: Request, res: Response): Promise<void> {
  const { progress, error } = await _taskProgress(req.body.sprintId);
  if (error) {
    res.status(400).send(error);
    return;
  }
